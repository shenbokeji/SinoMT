...
while(1)
{
  ...
  spibuf[0] = SPI_xferOneChar(SPI_MEM_CMD_JEDEC_ID);
  //DEBUG_printHexInt(spibuf);
  //DEBUG_printString(".\r\n");


  // Send dummy data, receive manufacture ID
  spibuf[1]  = SPI_xferOneChar(0x01);
  //DEBUG_printHexInt(spibuf);
  //DEBUG_printString(".\r\n");

  {
    ...
    spibuf[2]  = SPI_xferOneChar(0x02);
	//DEBUG_printHexInt(spibuf);
	//DEBUG_printString(".\r\n");

    // Send dummy data, receive manufacture ID
    spibuf[3]  = SPI_xferOneChar(0x03);
	  //DEBUG_printHexInt(spibuf);
  //DEBUG_printString(".\r\n");
  }
  DEBUG_printString("ID 0=\r\n");
  DEBUG_printHexInt(spibuf[1]);
  DEBUG_printString("ID 1=\r\n");
  DEBUG_printHexInt(spibuf[2]);
  DEBUG_printString("ID 2=\r\n");
  DEBUG_printHexInt(spibuf[3]);
  DEBUG_printString("\r\n");
  SPI_disableCS();
}
}
