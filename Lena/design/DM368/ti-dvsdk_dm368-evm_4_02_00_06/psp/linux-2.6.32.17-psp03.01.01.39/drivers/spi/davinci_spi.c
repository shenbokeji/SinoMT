...
static void davinci_spi_chipselect(struct spi_device *spi, int value)
{
	...
	if (value == BITBANG_CS_INACTIVE) {
		...
		if(pdata->use_dma==1) gpio_direction_output(25,0);
		else if(pdata->use_dma==0) gpio_direction_output(37,0);	
	}
}
...
//add 
	...
	clear_io_bits(davinci_spi->base + SPIINT,0xFFFFFFFF);//mask all int
