...
int saveenv(void)
{
	...
	if (CFG_ENV_SECT_SIZE > CFG_ENV_SIZE) {
		saved_size = CFG_ENV_SECT_SIZE - CFG_ENV_SIZE;
		saved_offset = CFG_ENV_OFFSET + CFG_ENV_SIZE;
	...
	ret = spi_flash_erase(env_flash, CFG_ENV_OFFSET, sector * CFG_ENV_SECT_SIZE);
	if (ret)
		goto done;

	puts("Writing to SPI flash...");
	ret = spi_flash_write(env_flash, CFG_ENV_OFFSET, CFG_ENV_SIZE, env_ptr);
	if (ret)
		goto done;
