...
typedef struct {
	...
	ulong	erase_size;		/* bytes number of one erase block */
	ulong	flash_id;		         /* combined device & manufacturer code	*/
	ulong	start[CFG_MAX_FLASH_SECT];   /* physical sector start addresses */
	uchar	protect[CFG_MAX_FLASH_SECT]; /* sector protection status	*/
#ifdef CFG_FLASH_CFI
#ifdef CONFIG_OS
       void   *flsem; /* lock for read/write */
#endif
...
#ifdef CFG_FLASH_CFI
extern unsigned long BspFlashErase(unsigned long ulOffset,unsigned long ulLength);
extern unsigned long BspFlashRead(unsigned long ulOffset, unsigned char *pucBufAddr, unsigned long ulDataLen);
extern unsigned long BspFlashWrite(unsigned long ulOffset, unsigned char *pucBufAddr, unsigned long ulDataLen);
#endif
/*-----------------------------------------------------------------------
...
#define ERR_READ_GET_HANDLE       129
...
#define FLASH_S25FL512S  0x00F2		/* Spansion S25FL512S			*/
...
#define FLASH_MAN_SPANSION 0x00070000 /* Spansion */
...
#if !defined(CFG_NO_FLASH) && defined(CFG_FLASH_CFI) && defined(CONFIG_OS) && !defined(CONFIG_MPC8548_H8548)
#define CFI_FLASH_SEM_INIT {if(flash_sem_init ())printf("cfi_flash semaphore initialization failed!\n");}
#endif
#ifndef CFI_FLASH_SEM_INIT
#define CFI_FLASH_SEM_INIT
#endif
